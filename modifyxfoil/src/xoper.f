...
C---- set final Mach, CL, Cp distributions, and hinge moment
   ...
   53 CONTINUE
	  ...
	  CLOSE(516)
	  CLOSE(517)
C	   OPEN(114,FILE = 'DCLDG.DAT', STATUS = 'REPLACE')
C	   OPEN(115,FILE = 'DCMDG.DAT', STATUS = 'REPLACE')
C	   DO 51 I = 1, N
C		 DO 52 J = 1, N
C	   	 TEM(J) = GAM(J)
C		 TAM(J) = GAM_A(J)
C   52    CONTINUE
C		 TEM(I) = COSA*(GAMU(I,1)+0.001) + SINA*(GAMU(I,2)+0.001)
C         TAM(I) = -SINA*(GAMU(I,1)+0.001) + COSA*(GAMU(I,2)+0.001)
C		 CALL CLCALC(N,X,Y,TEM,TAM,ALFA,MINF,QINF, XCMREF,YCMREF,
C     &            CL,CM,CDP, CL_ALF,CL_MSQ)
C		 BRUL = (CL-CLB)/(TEM(I)-GAM(I))
C		 BRUM = (CM-CMB)/(TEM(I)-GAM(I))
C		 WRITE(114,*) BRUL
C		 WRITE(115,*) BRUM
C   51  CONTINUE
C	   CLOSE(114)
